...
for(i=0; i<graduates.length; i++){
  if(graduates[i] == graduates.indexOf('Uni')){
    return unisWithUni.push(graduates[i]);
  }};
...
/*    Request 1: .forEach()
The zoos wants to display both scientific and animal name in the habitats. Populate the displayNames array with animal_name and scientific_name of each animal. displayNames will be an array of strings, and each string should follow this pattern: "Name: Jackal, asiatic, Scientific: Canis aureus."
*/

const displayNames = [];
zooAnimals.forEach(function(zooAnimals){
  return displayNames.push(`Name: ${zooAnimals.animal_name}, Sicentific: ${zooAnimals.scientific_name}`);
})
console.log(displayNames);


/*    Request 2: .map()
The zoos need a list of all their animal names converted to lower case. Using map, create a new array of strings named lowCaseAnimalNames, each string following this pattern: "jackal, asiatic". Log the resut.
*/
...
zooAnimals.filter(function(zooAnimals){
  if(zooAnimals.population < 5){
    return lowPopulationAnimals.push(zooAnimals[i])
  }
})
...
var initialValue = 0;
var sum = zooAnimals.population.reduce(
  (accumulator, currentValue) => accumulator + currentValue.population, initialValue);
...
/*
Stretch: Convert your array method callbacks into arrow functions.
*/